class Solution {
    ...
    func isAlphaNum (_ char: Character) -> Bool {
        if ("A".unicodeScalars.first!.value <= char.unicodeScalars.first!.value && "Z".unicodeScalars.first!.value >= char.unicodeScalars.first!.value) 
        || ("a".unicodeScalars.first!.value <= char.unicodeScalars.first!.value && "z".unicodeScalars.first!.value >= char.unicodeScalars.first!.value)
        || ("0".unicodeScalars.first!.value <= char.unicodeScalars.first!.value && "9".unicodeScalars.first!.value >= char.unicodeScalars.first!.value) {
            return true
        } else {
            return false
        }
